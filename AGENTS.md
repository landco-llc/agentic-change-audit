...
The Controller starts each work item with a fresh read of its record, the fixed
base, the expected head, the allowed scope, and the required evidence. Route
implementation, independent audit, correction, and fresh re-audit to separate
agents. An audit is fixed-head and read-only: a correction changes the head and
therefore requires a new independent re-audit.
...
work-result contracts and the control-state model.
